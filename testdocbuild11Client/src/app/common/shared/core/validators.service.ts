...
export class ValidatorsService {
  ...
  static integer(control: AbstractControl) {
    ...
    let reg = /^[0-9]\d*$/;
    let isValid = !isNaN(num) && reg.test(num.toString());
    const message = {
      integer: {
  ...
  static sqlQuery(control: AbstractControl) {
    // let reg = /select\s+(.*)\s+from\s+(.*)\s+where\s+?(.*)?/i;
    let reg = /select\s+(.*)\s+from\s+(.*)/i;

    if (!control.value) {
      return null;
    }
    let isValid = control.value && reg.test(control.value.toString());
    const message = {
      sqlQuery: {
  ...
  static percentageValidation(control: AbstractControl) {
    let reg = /(^100(\.0{1,2})?$)|(^([1-9]([0-9])?|0)(\.[0-9]{1,2})?$)/;
    if (!control.value) {
      return null;
    }
    let isValid = control.value && reg.test(control.value.toString());
  ...
  static websiteValidate(control: AbstractControl) {
    let reg = /^(http:\/\/|https:\/\/)?(www.)?([a-zA-Z0-9]+).[a-zA-Z0-9]*.[a-z]{3}.?([a-z]+)?$/;
    if (!control.value) {
      return null;
    }
    let isValid = control.value && reg.test(control.value.toString());
