...
import { ICountry, CountryService, CountryDetailsComponent, CountryListComponent, CountryNewComponent } from '../';

describe('CountryListComponent', () => {
  let fixture: ComponentFixture<CountryListComponent>;
  let component: CountryListComponent;
  let el: HTMLElement;
  let constData: ICountry[] = [
    {
      country: 'country1',
      countryId: 1,
      lastUpdate: new Date(),
  ...
  describe('Unit tests', () => {
    beforeEach(async(() => {
      TestBed.configureTestingModule({
        declarations: [CountryListComponent, ListComponent],
        imports: [TestingModule],
        providers: [CountryService, ChangeDetectorRef],
  ...
  describe('Integration tests', () => {
    ...
    it('should verify that redirected to details page when edit button is clicked', async () => {
      ...
      expect(decodeURIComponent(location.path())).toBe(
        `/country/${ServiceUtils.encodeIdByObject(data[0], component.primaryKeys)}`
      );
    });
