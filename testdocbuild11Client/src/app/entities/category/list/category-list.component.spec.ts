...
import { ICategory, CategoryService, CategoryDetailsComponent, CategoryListComponent, CategoryNewComponent } from '../';

describe('CategoryListComponent', () => {
  let fixture: ComponentFixture<CategoryListComponent>;
  let component: CategoryListComponent;
  let el: HTMLElement;
  let constData: ICategory[] = [
    {
      categoryId: 1,
      lastUpdate: new Date(),
      name: 'name1',
  ...
  let data: ICategory[] = [...constData];

  describe('Unit tests', () => {
    beforeEach(async(() => {
      TestBed.configureTestingModule({
        declarations: [CategoryListComponent, ListComponent],
        imports: [TestingModule],
        providers: [CategoryService, ChangeDetectorRef],
  ...
  describe('Integration tests', () => {
    ...
    it('should verify that redirected to details page when edit button is clicked', async () => {
      ...
      expect(decodeURIComponent(location.path())).toBe(
        `/category/${ServiceUtils.encodeIdByObject(data[0], component.primaryKeys)}`
      );
    });
