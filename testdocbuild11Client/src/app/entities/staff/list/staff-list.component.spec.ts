...
import { IStaff, StaffService, StaffDetailsComponent, StaffListComponent, StaffNewComponent } from '../';

describe('StaffListComponent', () => {
  let fixture: ComponentFixture<StaffListComponent>;
  let component: StaffListComponent;
  let el: HTMLElement;
  let constData: IStaff[] = [
    {
      active: true,
      email: 'email1',
  ...
  describe('Unit tests', () => {
    beforeEach(async(() => {
      TestBed.configureTestingModule({
        declarations: [StaffListComponent, ListComponent],
        imports: [TestingModule],
        providers: [StaffService, ChangeDetectorRef],
  ...
  describe('Integration tests', () => {
    ...
    it('should verify that redirected to details page when edit button is clicked', async () => {
      ...
      expect(decodeURIComponent(location.path())).toBe(
        `/staff/${ServiceUtils.encodeIdByObject(data[0], component.primaryKeys)}`
      );
    });
