...
import { IActor, ActorService, ActorDetailsComponent, ActorListComponent, ActorNewComponent } from '../';

describe('ActorListComponent', () => {
  let fixture: ComponentFixture<ActorListComponent>;
  let component: ActorListComponent;
  let el: HTMLElement;
  let constData: IActor[] = [
    {
      actorId: 1,
  ...
  let data: IActor[] = [...constData];

  describe('Unit tests', () => {
    beforeEach(async(() => {
      TestBed.configureTestingModule({
        declarations: [ActorListComponent, ListComponent],
        imports: [TestingModule],
        providers: [ActorService, ChangeDetectorRef],
  ...
  describe('Integration tests', () => {
    ...
    it('should verify that redirected to details page when edit button is clicked', async () => {
      ...
      expect(decodeURIComponent(location.path())).toBe(
        `/actor/${ServiceUtils.encodeIdByObject(data[0], component.primaryKeys)}`
      );
    });
