...
import { IUser, UserService, UserDetailsComponent, UserListComponent, UserNewComponent } from '../';
  ...
  describe('Unit tests', () => {
    beforeEach(async(() => {
      TestBed.configureTestingModule({
        declarations: [UserListComponent, ListComponent],
        imports: [TestingModule],
        providers: [UserService, ChangeDetectorRef],
  ...
  describe('Integration tests', () => {
    ...
    it('should verify that redirected to details page when edit button is clicked', async () => {
      ...
      expect(decodeURIComponent(location.path())).toBe(
        `/user/${ServiceUtils.encodeIdByObject(data[0], component.primaryKeys)}`
      );
    });
