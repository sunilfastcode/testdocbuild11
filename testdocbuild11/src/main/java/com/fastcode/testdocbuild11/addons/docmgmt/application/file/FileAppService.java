...
import com.fastcode.testdocbuild11.addons.docmgmt.domain.file.IFileManager;
import com.fastcode.testdocbuild11.addons.docmgmt.domain.file.QFileEntity;
...
public class FileAppService implements IFileAppService {
    ...
    public CreateFileOutput create(CreateFileInput input) {
        ...
        FileEntity createdFile = _fileManager.create(file);
    ...
    public UpdateFileOutput update(Long fileId, UpdateFileInput input) {
        ...
        FileEntity updatedFile = _fileManager.update(file);
        return mapper.fileEntityToUpdateFileOutput(updatedFile);
    }

    @Transactional(propagation = Propagation.REQUIRED)
    public void delete(Long fileId) {
        FileEntity existing = _fileManager.findById(fileId);
        _fileManager.delete(existing);
    }

    @Transactional(propagation = Propagation.NOT_SUPPORTED)
    public FindFileByIdOutput findById(Long fileId) {
        FileEntity foundFile = _fileManager.findById(fileId);
    ...
    public List<FindFileByIdOutput> find(SearchCriteria search, Pageable pageable) throws Exception {
        Page<FileEntity> foundFile = _fileManager.findAll(search(search), pageable);
